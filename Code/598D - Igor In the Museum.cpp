        ...
        for (int j = 0; j < m; j++)
        {
            if ((museum[i][j] == '.') && (vis[i][j] == 0))
            {
                ...
                while (!q.empty())
                {
                    ...
                    for (int k = 0; k < 4; k++)
                    {
                        if (museum[u + move[k].first][v + move[k].second] == '*')
                            cnt++;
                        else
                        {
                            if (vis[u + move[k].first][v + move[k].second] == 0)
                            {
                                vis[u + move[k].first][v + move[k].second] = itr;
                                q.push({u + move[k].first, v + move[k].second});
                            }
                        }
                    }
                }
